...
export const useSocialPaymentsStore = create<SocialPaymentsStore>((set) => ({
  contacts: [
    {
      id: "contact-1",
      name: "Alex Johnson",
  ...
  addContact: (contact) =>
    set((state) => ({
      contacts: [...state.contacts, { id: `contact-${Date.now()}`, ...contact }],
    })),
  ...
  addGroup: (group) =>
    set((state) => ({
      groups: [...state.groups, { id: `group-${Date.now()}`, ...group }],
    })),
  ...
  addActivity: (activity) =>
    set((state) => ({
      activities: [{ id: `activity-${Date.now()}`, ...activity }, ...state.activities],
    })),
